...
from amazon_override.overrides.amazon_sp_api import (
    SPAPI,
    CatalogItems,
    Finances,   
    Orders,
...
from amazon_override.overrides.amazon_sp_api_settings import (
    AmazonSPAPISettings,
)
...
class AmazonRepository:
    ...
    def create_sales_order(self, order) -> str | None:
        ...
        if so:
            ...
            # Adding tax logic here, before dealing with charges and fees
            try:
    ...
    def get_orders(self, created_after) -> list:
        ...
        orders = self.get_orders_instance()
        frappe.log_error("orders instance created", orders)
        order_statuses = [
            "PendingAvailability",
            "Pending",
            "Unshipped",
        ...
        frappe.log_error(
            message=f"Orders Payload: {orders_payload}", title="Amazon Orders Payload"
        )
